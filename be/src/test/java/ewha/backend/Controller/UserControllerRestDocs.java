...
//
// 	@Test
// 	void verifyDtoRejectTest() throws Exception {
//
// 		String content = gson.toJson(VERIFY_REJECT_DTO);
//
// 		given(userService.verifyVerifyDto(Mockito.any(UserDto.Verify.class))).willReturn(List.of(List.of("A")));
// 		given(userMapper.listToVerifyResponse(anyList())).willReturn(VERIFY_RESPONSE_LIST);
//
// 		ResultActions actions =
// 			mockMvc.perform(
// 				RestDocumentationRequestBuilders.post("/users/verification")
...
// 						fieldWithPath("[].field").type(JsonFieldType.STRING).description("실패한 필드"),
// 						fieldWithPath("[].rejectedValue").type(JsonFieldType.STRING).description("실패한 필드 값"),
// 						fieldWithPath("[].reason").type(JsonFieldType.STRING).description("실패 이유")
// 					)
// 				)));
// 	}
//
...
// 		ResultActions actions =
// 			mockMvc.perform(
// 				RestDocumentationRequestBuilders.patch("/users/firstlogin")
// 					.accept(MediaType.APPLICATION_JSON)
// 					.contentType(MediaType.APPLICATION_JSON)
// 					.content(content)
// 			);
